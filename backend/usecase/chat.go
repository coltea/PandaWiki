...
func (u *ChatUsecase) ChatRagOnly(ctx context.Context, req *domain.ChatRagOnlyRequest) (<-chan domain.SSEEvent, error) {
	...
	go func() {
		...
		_, rankedNodes, err := u.llmUsecase.GetRankNodes(ctx, kb.DatasetID, req.Message, groupIds, 0, nil)
		if err != nil {
...
func (u *ChatUsecase) Search(ctx context.Context, req *domain.ChatSearchReq) (*domain.ChatSearchResp, error) {
	...
	_, rankedNodes, err := u.llmUsecase.GetRankNodes(ctx, kb.DatasetID, req.Message, groupIds, 0.1, nil)
	if err != nil {
		return nil, err
	}
